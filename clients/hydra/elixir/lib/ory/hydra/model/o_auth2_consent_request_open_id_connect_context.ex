# NOTE: This file is auto generated by OpenAPI Generator 6.0.1 (https://openapi-generator.tech).
# Do not edit this file manually.

defmodule Ory.Hydra.Model.OAuth2ConsentRequestOpenIdConnectContext do
  @moduledoc """
  
  """

  @derive [Poison.Encoder]
  defstruct [
    :acr_values,
    :display,
    :id_token_hint_claims,
    :login_hint,
    :ui_locales
  ]

  @type t :: %__MODULE__{
    :acr_values => [String.t] | nil,
    :display => String.t | nil,
    :id_token_hint_claims => %{optional(String.t) => AnyType} | nil,
    :login_hint => String.t | nil,
    :ui_locales => [String.t] | nil
  }
end

defimpl Poison.Decoder, for: Ory.Hydra.Model.OAuth2ConsentRequestOpenIdConnectContext do
  import Ory.Hydra.Deserializer
  def decode(value, options) do
    value
    |> deserialize(:id_token_hint_claims, :map, Ory.Hydra.Model.AnyType, options)
  end
end

